...
** Con este comando ya no es necesario escribir la versión del PHP, puesto que te modifica la que tienes instalada en tu computador.
```
sudo apt install -y php-{curl,gd,imagick,intl,mbstring,xml,zip}
```
image::assets/imgs/wordpress_instalacion9.png[wordpress9]

** En caso de que se prefiera hacer con el número de versión, tienes estas dos opciones:
```
